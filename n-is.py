...
def init_sound(selected_music=None):
    ...
    try:
        ...
        if selected_music == 'korobeiniki - piano':
            music_file = os.path.join(music_folder, "tetris-theme-korobeiniki-arranged-for-piano-186249.mp3")
            vol = 0.7
        elif selected_music == 'korobeiniki - music box':
            music_file = os.path.join(music_folder, "tetris-theme-korobeiniki-rearranged-arr-for-music-box-184978.mp3")
            vol = 0.3
        elif selected_music == 'korobeiniki - strings':
            music_file = os.path.join(music_folder, "tetris-theme-korobeiniki-rearranged-arr-for-strings-185592.mp3")
            vol = 0.5
        else:
            music_file = os.path.join(music_folder, "Clair_de_lune_(Claude_Debussy)_Suite_bergamasque.ogg")
            vol = 0.95
    ...
    try:
        ...
        for i in range(frames):
            ...
            if wave_type == 'sine':
                ...
                wave = volume * np.sin(2 * np.pi * frequency * t)  # Default to sine
...
def sound_game_over():
    ...
    notes = [(523, 200, 0.15), (415, 250, 0.11), (294, 300, 0.07)]  # freq, duration, volume
...
def show_menu(stdscr):
    ...
    selected_ext = show_option_menu(stdscr, "Choose Polyomino Set", ext_options, ext_texts, selected_info)
    ...
    selected_mix = show_option_menu(stdscr, "Select Piece Variety", mix_options, mix_texts, mix_info)
    if selected_mix is None:
        return None, None, None, None

    # select music
    music_options = ['korobeiniki - piano', 'korobeiniki - music box', 'korobeiniki - strings', 'clair de lune']
    music_texts = ['Korobeiniki - Piano', 'Korobeiniki - Music Box', 'Korobeiniki - Strings', 'Clair de Lune']
    ...
    selected_music = show_option_menu(stdscr, "Select Music Theme", music_options, music_texts, music_info)
...
if args.n is None:
    ...
    try:
        selected_n, selected_ext, selected_mix, selected_music = curses.wrapper(show_menu)
        if selected_n is None:
            exit(0)
        args.n = selected_n
        args.e = selected_ext
...
def show_game_over_screen(stdscr, score):
    ...
    for i, line in enumerate(GAME_OVER_ART):
        if art_start_y + i < max_y - 1 and len(line) < max_x:
            ...
            stdscr.addstr(art_start_y + i, (max_x - len(line)) // 2, line, color | curses.A_BOLD)
    ...
    if stats_y < max_y - 4:
        stdscr.addstr(stats_y, (max_x - len(final_score_text)) // 2, final_score_text, curses.color_pair(8) | curses.A_BOLD)
        stdscr.addstr(stats_y + 1, (max_x - len(level_text)) // 2, level_text, curses.color_pair(7))
        stdscr.addstr(stats_y + 2, (max_x - len(lines_text)) // 2, lines_text, curses.color_pair(7))
    
    # Exit instruction
    exit_text = "Press 'Q' to quit"
    if stats_y + 4 < max_y - 1:
        stdscr.addstr(stats_y + 4, (max_x - len(exit_text)) // 2, exit_text, curses.color_pair(6) | curses.A_BLINK)
...
def show_pause_screen(stdscr):
    ...
    if inst_y < max_y - 3:
        stdscr.addstr(inst_y, (max_x - len(resume_text)) // 2, resume_text, curses.color_pair(10) | curses.A_BLINK)
        stdscr.addstr(inst_y + 1, (max_x - len(quit_text)) // 2, quit_text, curses.color_pair(10))
